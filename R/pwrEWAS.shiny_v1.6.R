  ...
  server = function(input,output){
    ...
    shiny::observeEvent(input$goButton, {
      ...
      shiny::withProgress(message = 'Program running.', detail = "Please wait!", value = NULL, {
        ...
        output$powerPlot <- shiny::renderPlot({isolate(pwrEWAS.powerPlot(out$powerArray, sd = ifelse(input$switchTargetDmSd == 'Target delta', FALSE, TRUE)))})
        ...
        # delta density plot
        output$deltaDensity = shiny::renderPlot({isolate(pwrEWAS.deltaDensity(out$deltaArray, input$detectionLimit, sd = ifelse(input$switchTargetDmSd == 'Target delta', FALSE, TRUE)))})
