...
pwrEWAS = function(minTotSampleSize, # min total sample size
  ...
  combine_tau = function(listA, listB){
    ...
    if(class(listA[["metric"]]$probTP) !="array" & class(listA[["metric"]]$probTP) !="matrix") listA[["metric"]]$probTP = matrix(listA[["metric"]]$probTP)
    if(class(listB[["metric"]]$probTP) !="array" & class(listB[["metric"]]$probTP) !="matrix") listB[["metric"]]$probTP = matrix(listB[["metric"]]$probTP)
  ...
  if(is.null(deltaSD)){
    ...
    # cat(paste("Taus: ",paste(tau, collapse=", "), "\n", sep = ""))
  } else {
    tau = deltaSD 
    K = NULL
  ...
  multiThreadOut = foreach(d = 1:length(tau), 
    ...
    foreach(Ntot = totSampleSizes, .combine = combine_totSampleSizes) %dopar% { 
      ...
      for(sim in 1:sims){
        ...
        V  = intersect(cpgIdxName[DMtest$fdr<FDRcritVal], G0) # FP
        Sa = intersect(cpgIdxName[DMtest$fdr<FDRcritVal], G1A) # NP
        Sb = intersect(cpgIdxName[DMtest$fdr<FDRcritVal], G1B) # TP
        R = cpgIdxName[DMtest$fdr<FDRcritVal]
        
        G0_V = intersect(cpgIdxName[!(DMtest$fdr<FDRcritVal)], G0) # TN
        G1A_Sa = intersect(cpgIdxName[!(DMtest$fdr<FDRcritVal)], G1A) # NN
        G1B_Sb = intersect(cpgIdxName[!(DMtest$fdr<FDRcritVal)], G1B) # FN
        ...
        probTP[sim] = ifelse(length(Sb)>0, 1, 0)
